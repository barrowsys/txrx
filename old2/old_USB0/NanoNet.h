...
class NanoNet {
  private:
    byte _txRate = 20; // In bits per second
...
#define OUTPUT_STATE() if(false) {}
  /* if(_state != LAST_STATE) {\ */
  /*   LAST_STATE = _state; \ */
  /*   if(_state == NN_IDLE) { Serial.println("State: NN_IDLE"); } \ */
  /*   else if(_state == NN_RX_WAITING) { Serial.println("State: NN_RX_WAITING"); } \ */
  /*   else if(_state == NN_RX_HEADER) { Serial.println("State: NN_RX_HEADER"); } \ */
  /*   else if(_state == NN_RX_PAYLOAD) { Serial.println("State: NN_RX_PAYLOAD"); } \ */
  /*   else if(_state == NN_RX_CRC) { Serial.println("State: NN_RX_CRC"); } \ */
  /*   else if(_state == NN_TX) { Serial.println("State: NN_TX"); } \ */
  /*   else { Serial.print("Unknown State: "); Serial.println(_state, DEC); } \ */
  /* } */

#define CHECK_STATE(EXPECTED) if(false) {}
  /* if(_state != LAST_STATE) {\ */
  /*   Serial.print("Expected State: "); \ */
  /*   Serial.println(EXPECTED); \ */
  /*   LAST_STATE = _state; \ */
  /*   if(_state == NN_IDLE) { Serial.println("Checked State: NN_IDLE"); } \ */
  /*   else if(_state == NN_RX_WAITING) { Serial.println("Checked State: NN_RX_WAITING"); } \ */
  /*   else if(_state == NN_RX_HEADER) { Serial.println("Checked State: NN_RX_HEADER"); } \ */
  /*   else if(_state == NN_RX_PAYLOAD) { Serial.println("Checked State: NN_RX_PAYLOAD"); } \ */
  /*   else if(_state == NN_RX_CRC) { Serial.println("Checked State: NN_RX_CRC"); } \ */
  /*   else if(_state == NN_TX) { Serial.println("Checked State: NN_TX"); } \ */
  /*   else { Serial.print("Unknown State: "); Serial.println(_state, DEC); } \ */
  /* } */
...
byte NanoNet::recieveFrame(char *buf) {
  ...
  while (true) {
    ...
    if (NN_new_bit[_register] == true) {
      /* Serial.print("Status: "); */
      /* Serial.print(_state == NN_RX_HEADER, BIN); */
      /* Serial.print(_state == NN_RX_PAYLOAD, BIN); */
      /* Serial.print(_state = NN_RX_CRC, BIN); */
      /* Serial.println(); */
      OUTPUT_STATE();
      if(_state == NN_RX_HEADER) {
      // Step 2
        CHECK_STATE("NN_RX_HEADER");
        bit_pos++;
        if(bit_pos == 8) {
        // Step 2.1
      ...
      /* Serial.println(NN_rx_buf[0].s, BIN); */
    }
  }
  _state = NN_IDLE;
}
